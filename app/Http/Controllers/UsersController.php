...
class UsersController extends Controller
{
    ...
    public function teams(Request $request){
        ...
        $dataForView['groupedUsers'] = JoiningHistory::select('level', DB::raw('count(user_id) as user_count'))
            ->where('for_user_id', $user->id) // Filter by for_user_id
            ->groupBy('level')
            ->get();
    ...
    public function getTeams(Request $request)
    {
        ...
        if(!empty($request->input('search.value'))) {
            ...
            $search = $request->input('search.value'); 
            $transactions =  JoiningHistory::where('for_user_id', $user->id) // Filter by for_user_id
            ...
            $totalFiltered = JoiningHistory::where('for_user_id', $user->id) // Filter by for_user_id
                    ->with('User')
                    ->Where('level', $trans_type)
                    ->Where('is_paid', $status)
            ...
            // Filter by for_user_id
            $query1->where('for_user_id', $user->id);
